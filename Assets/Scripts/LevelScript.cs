...
public class LevelScript : MonoBehaviour
{
    ...
    void Awake()
    {
        storyCSV = Resources.Load<TextAsset>("TextTemplates/sms_templates");
        ...
        using (StringReader reader = new StringReader(fullText))
        {
            ...
            while ((line = reader.ReadLine()) != null)
            {
                ...
                //replace "," with ";" within quotation marks
                ...
                //if the value contains ";" replace it back with "," and remove the quotation marks
                entry.incomingSMS = Utils.ReplaceAll(values[2], ';', ',', '\"').Replace("\"", "");
                entry.replyTemplate = Utils.ReplaceAll(values[3], ';', ',', '\"').Replace("\"", "");
                entry.replySMS = Utils.ReplaceAll(values[5], ';', ',', '\"').Replace("\"", "");
